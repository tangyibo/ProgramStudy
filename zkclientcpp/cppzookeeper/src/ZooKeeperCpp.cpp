...
ZkRet ZooKeeper::deleteNode(const std::string &path,int version)
{
        return deleteTheNode(path,version);
}
...
ZkRet ZooKeeper::createTheNode(int flag, const std::string &path, const std::string &value, char *rpath, int rpathlen, bool recursive)
{
	...
	if(ZNONODE == ret)
	{
		...
		if(zr.ok() || zr.nodeExist())
		{
			...
			{
				LOG_ERROR(("create node failed, path=%s, ret=[%d]%s", path.c_str(),ret, errorStr(ret)));
			}
			return ZkRet(ret);	
		}
		else
...
ZkRet ZooKeeper::deleteTheNode(const std::string &path,int version)
{
        assert(!path.empty());
        int ret=zoo_delete(zhandle_, path.c_str(),version);
        return ZkRet(ret);
}
